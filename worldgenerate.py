...
while not good_setting:
    setting = gpt_control.run_llm(prompts["create_setting"]["system_prompt"],
...
monsters = gpt_control.run_llm(prompts["create_monster_setting"]["system_prompt"],
...
town_map = gpt_control.run_llm(prompts["build_town_map"]["system_prompt"], prompts["build_town_map"]["user_prompt"].format(setting),
                               temperature=0.8, json=True)
print(town_map)
town_map_json = json.loads(town_map)

indoor_map = gpt_control.run_llm(prompts["add_indoor_areas"]["system_prompt"],
...
for indoor_node in indoor_map_json["nodes"]:
    fixed_connections = gpt_control.run_llm(prompts["connect_indoor_areas"]["system_prompt"],
...
field_map = gpt_control.run_llm(prompts["build_field_map"]["system_prompt"].format(town_map_json["gate_direction"]),
...
town_field_connection = gpt_control.run_llm(prompts["connect_map"]["system_prompt"].format("city_gate", "city_outskirts"),
...
starting_point = gpt_control.run_llm(prompts["find_starting_point"]["system_prompt"], town_map,
                                     temperature=0.3)

print(starting_point)

dungeon_map = gpt_control.run_llm(prompts["build_dungeon_map"]["system_prompt"],
...
dungeon_connections = gpt_control.run_llm(prompts["connect_map"]["system_prompt"].format("field_to_dungeon", "first_dungeon_node"),
...
npcs = gpt_control.run_llm(prompts["create_npcs"]["system_prompt"],
...
for node in town_map_json["nodes"]:
    ...
    description = gpt_control.run_llm(prompts["describe_node"]["system_prompt"], user_prompt, temperature=0.7)
...
for npc in npc_json["characters"]:
    ...
    character_details = gpt_control.run_llm(prompts["character_details"]["system_prompt"], user_prompt, temperature=0.8,
                                            json=True)
    ...
    backstory = gpt_control.run_llm(prompts["character_backstory"]["system_prompt"], user_prompt, temperature=0.8)
